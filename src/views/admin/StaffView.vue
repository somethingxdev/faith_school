...
const rows = ref<PersonRow[]>([
  { id: 1, firstName: 'Иван', lastName: 'Иванов', roles: ['Учитель', 'Продавец'] },
  { id: 2, firstName: 'Мария', lastName: 'Петрова', roles: ['Администратор'] },
  { id: 3, firstName: 'Олег', lastName: 'Смирнов', roles: ['Учитель'] },
])
...
const columns = [
  columnHelper.display({ id: 'index', header: () => '№', cell: (ctx) => String(ctx.row.index + 1) }),
  columnHelper.display({ id: 'name', header: () => 'Имя', cell: (ctx) => `${ctx.row.original.firstName} ${ctx.row.original.lastName}` }),
  columnHelper.display({ id: 'roles', header: () => 'Роли', cell: (ctx) => ctx.row.original.roles.join(', ') }),
...
function openAdd() {
  editing.value = { id: 0, firstName: '', lastName: '', roles: [] }
  isOpen.value = true
}
...
<template>
  <h1 class="text-4xl font-sans text-black mb-4">Управление пользователями</h1>
  <ManagementTabs />
  ...
  <div class="grid grid-cols-3 gap-5 mb-7.5">
    ...
    <Button class="w-full" @click="openAdd"><PlusIcon class="size-6" /> Добавить сотрудника</Button>
  </div>
  ...
  <Dialog v-model:open="isOpen">
    <DialogContent class="sm:max-w-[600px]">
      <DialogHeader>
        <DialogTitle>{{ editing?.id === 0 ? 'Добавить пользователя' : 'Редактировать пользователя' }}</DialogTitle>
      </DialogHeader>
